...
object GridService
    extends WebServiceHelper[GridObject, Error] with LazyLogging {
  ...
  def getRequestedCrop(gridId: ImageIdWithCrop): Future[Option[GridImage]] = {
    ...
    if (currentImageData.contains(gridId))
      Future.successful(currentImageData.get(gridId))
    else {
      getGrid(gridId).map { grid =>
        for {
          exports <- grid.data.exports
          export <- findExport(exports, gridId.crop) if export.assets.nonEmpty
        } yield {
          val image =
            GridImage(export.assets, grid.data.metadata, export.master)
          agent send { oldImageData =>
  ...
  private[services] def findExport(
      exports: List[Export], cropId: String): Option[Export] =
    exports.find(_.id == cropId)

  override val wsUrl: String = Config.gridConfig.apiUrl

  override def wsPreExecute(req: Request.Builder): Request.Builder =
    req.addHeader("X-Gu-Media-Key", Config.gridConfig.key)
