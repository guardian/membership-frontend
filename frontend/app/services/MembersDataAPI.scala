...
object MembersDataAPI {
  implicit val tierReads = Reads[Tier] {
    case JsString(s) =>
      Tier.slugMap
        .get(s.toLowerCase)
        .map(JsSuccess(_))
        .getOrElse(JsError(s"Unknown tier $s"))
    case _ => JsError("Expected a string representation of a tier")
  }

  implicit val attributesReads: Reads[Attributes] =
    ((JsPath \ "tier").read[Tier] and (JsPath \ "membershipNumber")
          .readNullable[String])(Attributes.apply _)

  case class Attributes(tier: Tier, membershipNumber: Option[String])

  object Attributes {
    def fromMember(member: Member) =
      Attributes(member.subscription.plan.tier, member.contact.regNumber)
  }

  case class ApiError(message: String, details: String)
      extends RuntimeException(s"$message - $details")

  implicit val errorReads: Reads[ApiError] =
    ((JsPath \ "message").read[String] and (JsPath \ "details").read[String])(
        ApiError)

  case class Helper(cookies: AccessCredentials.Cookies)
      extends WebServiceHelper[Attributes, ApiError] {
  ...
  object Service {
    def checkMatchesResolvedMemberIn(memberRequest: SubReqWithSub[_]) =
      memberRequest.user.credentials match {
        case cookies: AccessCredentials.Cookies =>
          get(cookies).onComplete {
            case Success(memDataApiAttrs) =>
              val prefix =
                s"members-data-api-check identity=${memberRequest.user.id} salesforce=${memberRequest.subscriber.contact.salesforceContactId} : "
              val salesforceAttrs =
                Attributes.fromMember(memberRequest.subscriber)
              if (memDataApiAttrs != salesforceAttrs) {
                val message =
                  s"$prefix MISMATCH salesforce=$salesforceAttrs mem-data-api=$memDataApiAttrs"
                if (memDataApiAttrs.tier != salesforceAttrs.tier)
                  Logger.error(message) else Logger.warn(message)
            ...
            case Failure(err) =>
              Logger.error(
                  s"Failed while querying the members-data-api (OK in dev)",
                  err)
          }
        case _ =>
          Logger.error(
              s"Unexpected credentials! ${memberRequest.user.credentials}")
      }

    private def get(cookies: AccessCredentials.Cookies) =
      Helper(cookies).get[Attributes]("user-attributes/me/membership")
  }
}
