...
object TouchpointBackend {
  ...
  def forUser(user: Contact[_, _]): TouchpointBackend = if (isTestUser(user)) TestUser else Normal
}
...
case class TouchpointBackend(memberRepository: FrontendMemberRepository,
  ...
  def cancelSubscription(member: Contact[Member, _], user: IdMinimalUser, campaignCode: Option[String] = None): Future[String] = {
    for {
  ...
  def downgradeSubscription(member: Contact[Member, _], user: IdMinimalUser, campaignCode: Option[String] = None): Future[String] = {
    for {
