...
case class MasterclassData(
    eventId: String, webUrl: String, images: Option[ResponsiveImageGroup])
...
object MasterclassDataExtractor {
  ...
  def extractEventbriteInformation(content: Content): Seq[MasterclassData] = {
    val eventbriteIdsFromRefs = content.references
      .filter(_.`type` == "eventbrite")
      .map(_.id.stripPrefix("eventbrite/"))

    val eventbriteIds =
      (eventbriteIdsFromRefs ++ scrapeEventbriteIdsFrom(content)).distinct

    eventbriteIds.map(eventId =>
          MasterclassData(eventId,
                          content.webUrl,
                          ResponsiveImageGroup.fromContent(content)))
  }

  def scrapeEventbriteIdsFrom(content: Content): Seq[String] =
    for {
