...
class MemberService(identityService: IdentityService,
  ...
  override def createMember(
    ...
    Timing.record(salesforceService.metrics, "createMember") {
      formData.password.foreach(identityService.updateUserPassword(
          _, identityRequest, user.id))
      ...
      contactId.map { cId =>
        identityService.updateUserFieldsBasedOnJoining(user,
                                                       formData,
                                                       identityRequest)
  ...
  override def upgradeFreeSubscription(
    ...
    (for {
      customer <- stripeService.Customer.create(
          subscriber.contact.identityId, form.payment.token).liftM
  ...
  override def downgradeSubscription(
    ...
    def effectiveFrom(sub: model.PaidSubscription): DateTime =
      sub.chargedThroughDate
        .getOrElse(sub.firstPaymentDate).toDateTimeAtCurrentTime
  ...
  override def cancelSubscription(
      subscriber: com.gu.memsub.Subscriber.Member): Future[MemberError \/ Unit] = {
    (for {
      sub <- EitherT(subOrPendingAmendError(subscriber.subscription))
      cancelDate = sub match {
        case p: Subscription with PaidPS[Plan] =>
          p.chargedThroughDate.map(_.toDateTimeAtCurrentTime)
  ...
  override def previewUpgradeSubscription(
      subscription: PaidSubscription,
      newRatePlanId: ProductRatePlanId): Future[Seq[PreviewInvoiceItem]] = {
    for {
      result <- zuoraService.upgradeSubscription(subscription = subscription,
  ...
  override def getMembershipSubscriptionSummary(
    ...
    def plan(sub: Subscription): (Price, BillingPeriod) =
      sub match {
  ...
  override def getUsageCountWithinTerm(
    ...
    zuoraService.getUsages(subscription.name, unitOfMeasure, startDate).map {
      usages =>
        ...
        if (!hasComplimentaryTickets) None
        else Some(usages.size)
    }
  }
  ...
  override def createFreeSubscription(
    ...
    val currency = catalog.unsafeFindFree(planId)
    ...
    for {
      ...
      result <- zuoraService.createSubscription(
          subscribeAccount = SoapSubscribeAccount
            .stripe(contactId, currency, autopay = false),
          paymentMethod = None,
  ...
  override def createPaidSubscription(
    ...
    val validPromoCode = for {
      ...
      promotion <- promoService.findPromotion(code) if promotion.validateFor(
                      planId, joinData.zuoraAccountAddress.country).isRight
    } yield code
    ...
    for {
      ...
      plan = RatePlan(
          planId.get,
          None,
          featuresPerTier(zuoraFeatures)(planId, joinData.featureChoice)
            .map(_.id.get))
      currency = catalog.unsafeFindPaid(planId)
        .currencyOrGBP(joinData.zuoraAccountAddress.country)
      result <- zuoraService.createSubscription(
          subscribeAccount = SoapSubscribeAccount.stripe(contactId = contactId,
                                                         currency = currency,
                                                         autopay = true),
  ...
  private def upgradeSubscription(
    ...
    addressDetails.foreach(identityService.updateUserFieldsBasedOnUpgrade(
        contact.identityId, _, identityRequest))

    (for {
      _ <- salesforceService.updateMemberStatus(
          IdMinimalUser(contact.identityId, None), tier, customerOpt).liftM
      ...
      _ <- zuoraService.upgradeSubscription(
          sub, newPlan.productRatePlanId, featureIds, preview = false).liftM
    } yield {
