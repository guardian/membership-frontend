...
object MemberService {
  ...
  def featureIdsForTier(features: Seq[SoapQueries.Feature])(
      tier: Tier, choice: Set[FeatureChoice]): Seq[Feature.Id] = {
    def chooseFeature(choices: Set[FeatureChoice]): Seq[Feature.Id] =
      features.filter(f => choices.map(_.zuoraCode).contains(f.code)).map(_.id)
  ...
  def getRatePlanIdsToRemove(current: Seq[rest.RatePlan],
                             ...
                             discounts: DiscountRatePlanIds) =
    current.collect {
      case discount
          if discount.productRatePlanId == discounts.percentageDiscount.planId.get =>
        discount.id
      case plan
          if planFinder(ProductRatePlanId(plan.productRatePlanId)).isDefined =>
        plan.id
    }
}
...
class MemberService(
    ...
    extends api.MemberService with ActivityTracking {
    ...
    Timing
      .record(salesforceService.metrics, "createMember") {
        ...
        val contactId = formData match {
          case paid: PaidMemberJoinForm =>
            for {
              customer <- stripeService.Customer.create(user.id,
                                                        paid.payment.token)
              cId <- createContact
              subscription <- createPaidSubscription(
                  cId, paid, customer, campaignCode)
              updatedMember <- salesforceService.updateMemberStatus(
                  user, tier, Some(customer))
            } yield cId
          case _ =>
            for {
              cId <- createContact
        ...
        contactId.map { cId =>
          identityService.updateUserFieldsBasedOnJoining(
              user, formData, identityRequest)
      ...
      .andThen {
        case Success(contactId) =>
          logger.debug(
              s"createMember() success user=${user.id} memberAccount=$contactId")
          fromEventId.flatMap(EventbriteService.getBookableEvent).foreach {
            event =>
              event.service.wsMetrics.put(s"join-${tier.name}-event", 1)

              val memberData =
                MemberData(salesforceContactId = contactId.salesforceContactId,
                           identityId = user.id,
                           tier = tier,
                           campaignCode = campaignCode)

              track(EventActivity("membershipRegistrationViaEvent",
                                  Some(memberData),
                                  EventData(event)),
                    user)
          }
  ...
  override def upgradeFreeSubscription(
    ...
    (for {
      customer <- stripeService.Customer
        .create(subscriber.contact.identityId, form.payment.token)
        .liftM
  ...
  override def upgradePaidSubscription(
    ...
    (for {
      memberId <- EitherT(upgradeSubscription(
              subscriber.subscription,
              contact = subscriber.contact,
              planChoice = PaidPlanChoice(
                    newTier, subscriber.subscription.plan.billingPeriod),
              form = form,
              customerOpt = None,
              identityRequest = identityRequest,
              campaignCode = campaignCode
  ...
  override def downgradeSubscription(
    ...
    def effectiveFrom(sub: model.PaidSubscription) =
      sub.chargedThroughDate.getOrElse(sub.firstPaymentDate)
    ...
    (for {
      ...
      result <- zuoraService
        .downgradePlan(subscription = paidSub,
      ...
      track(
          MemberActivity(
              ...
              MemberData(
                  ...
                  tierAmendment = Some(DowngradeAmendment(
                            subscriber.subscription.plan.tier)) //getting effective date and subscription annual / month is proving difficult
              )),
          subscriber.contact)
    }).run
  }

  override def cancelSubscription(subscriber: com.gu.memsub.Subscriber.Member
      ): Future[MemberError \/ Unit] = {
    (for {
      sub <- EitherT(subOrPendingAmendError(subscriber.subscription))
      cancelDate = sub match {
        case p: Subscription with PaidPS [Plan] =>
          p.chargedThroughDate.getOrElse(DateTime.now.toLocalDate)
      ...
      track(MemberActivity("cancelMembership",
                           MemberData(subscriber.contact.salesforceContactId,
                                      subscriber.contact.identityId,
                                      subscriber.subscription.plan.tier)),
            subscriber.contact)
      track(MemberActivity("cancelMembership",
                           MemberData(subscriber.contact.salesforceContactId,
                                      subscriber.contact.identityId,
  ...
  override def previewUpgradeSubscription(subscription: PaidSubscription,
                                          newRatePlanId: ProductRatePlanId
      ): Future[MemberError \/ BillingSchedule] = {
    (for {
      _ <- EitherT(subOrPendingAmendError(subscription))
      result <- EitherT(zuoraService
            .upgradeSubscription(
                Amend(subscriptionId = subscription.id.get,
                      plansToRemove = Seq(subscription.ratePlanId),
                      newRatePlans = NonEmptyList(
                            RatePlan(newRatePlanId.get, None)),
                      previewMode = true))
            .map(\/.right))
    } yield
      BillingSchedule
        .fromPreviewInvoiceItems(result.invoiceItems)
        .getOrElse(
            throw new IllegalStateException(
                s"Sub ${subscription.id} upgrading to $newRatePlanId has no bills")
        )).run
  }

  def subscriptionUpgradableTo(sub: Subscription with PaymentStatus[Plan],
    ...
    catalog.find(sub.productRatePlanId) exists { currentPlan =>
      ...
      !sub.isInTrialPeriod && targetCurrencies.contains(sub.currency) &&
      newPlan.tier > currentPlan.tier
    }
  }

  override def getMembershipSubscriptionSummary(
    ...
    def price(amount: Float)(implicit currency: Currency) =
    ...
    def getSummaryViaInvoice =
      for {
        ...
        val nextPayment = Some(NextPayment(
                price(payment.current.price), payment.current.nextPaymentDate))
    ...
    for {
      ...
      summary <- if (userInvoiced) getSummaryViaInvoice
                else getSummaryViaPreview
    } yield summary
  }

  override def getUsageCountWithinTerm(
      subscription: Subscription,
      unitOfMeasure: String): Future[Option[Int]] = {
    val features = subscription.features
    val startDate = subscription.startDate
      .toDateTimeAtStartOfDay(DateTimeZone.forID("America/Los_Angeles"))
    zuoraService.getUsages(subscription.name, unitOfMeasure, startDate).map {
      usages =>
        val hasComplimentaryTickets =
          features.map(_.code).contains(FreeEventTickets.zuoraCode)
        if (!hasComplimentaryTickets) None else Some(usages.size)
    }
  }

  override def recordFreeEventUsage(subs: Subscription,
                                    event: RichEvent,
                                    order: EBOrder,
    ...
    for {
      ...
      logger.info(
          s"Recorded a complimentary event ticket usage for account ${subs.accountId}, subscription: ${subs.name}, details: $description")
      result
    }
  }

  override def retrieveComplimentaryTickets(
      sub: Subscription, event: RichEvent): Future[Seq[EBTicketClass]] = {
    Timing.record(salesforceService.metrics, "retrieveComplimentaryTickets") {
      for {
        usageCount <- getUsageCountWithinTerm(
            sub, FreeEventTickets.unitOfMeasure)
      } yield {
        val hasComplimentaryTickets = usageCount.isDefined
        val allowanceNotExceeded =
          usageCount.exists(_ < FreeEventTickets.allowance)
        logger.info(
            s"User ${sub.accountId} has used $usageCount tickets" ++ s"(allowance not exceeded: $allowanceNotExceeded, is entitled: $hasComplimentaryTickets)")

        logger.info(
            s"Complementary tickets: ${event.internalTicketing.map(_.complimentaryTickets)}")
  ...
  override def createEBCode(subscriber: com.gu.memsub.Subscriber.Member,
                            event: RichEvent): Future[Option[EBCode]] = {
    retrieveComplimentaryTickets(subscriber.subscription, event).flatMap {
      complimentaryTickets =>
        val code = DiscountCode.generate(
            s"A_${subscriber.contact.identityId}_${event.id}")
        val unlockedTickets =
          complimentaryTickets ++ event.retrieveDiscountedTickets(
              subscriber.subscription.plan.tier)
        event.service.createOrGetAccessCode(event, code, unlockedTickets)
    }
  }

  override def createFreeSubscription(
      contactId: ContactId, joinData: JoinForm): Future[SubscribeResult] = {
    val planId = joinData.planChoice.productRatePlanId
    val currency = catalog
      .unsafeFindFree(planId)
      .currencyOrGBP(joinData.deliveryAddress.country)
  ...
  override def createPaidSubscription(
      contactId: ContactId,
    ...
    val subscribe =
      zuoraService.getFeatures map { features =>
        val planId = joinData.planChoice.productRatePlanId
        val plan =
          RatePlan(planId.get,
                   None,
                   featuresPerTier(features)(planId, joinData.featureChoice)
                     .map(_.id.get))
        val currency = catalog
          .unsafeFindPaid(planId)
          .currencyOrGBP(joinData.zuoraAccountAddress.country)

        Subscribe(account = Account.stripe(contactId = contactId,
                                           currency = currency,
                                           autopay = true),
                  paymentMethod = CreditCardReferenceTransaction(
                        customer.card.id, customer.id).some,
    ...
    subscribe
      .map(promoService.applyPromotion(
              _,
              joinData.suppliedPromoCode,
              joinData.zuoraAccountAddress.country.some))
      .flatMap(zuoraService.createSubscription)
  }

  private def featuresPerTier(zuoraFeatures: Seq[SoapQueries.Feature])(
      productRatePlanId: ProductRatePlanId,
      choice: Set[FeatureChoice]): Seq[SoapQueries.Feature] = {
  ...
  def latestInvoiceItems(
      items: Seq[SoapQueries.InvoiceItem]): Seq[SoapQueries.InvoiceItem] = {
  ...
  private def upgradeSubscription(
      sub: MembershipSub,
      contact: Contact,
    ...
    val oldRest = zuoraService
      .getRestSubscription(sub.name)
      .map(_.getOrElse(
              throw new Exception(s"REST sub not found for ${sub.id}")
          ))

    val zuoraFeatures = zuoraService.getFeatures.map { fs =>
      featureIdsForTier(fs)(tier, form.featureChoice)
    }
    val newRatePlan = zuoraFeatures.map(
        fs => RatePlan(newPlan.productRatePlanId.get, None, fs.map(_.get)))

    val country = identityService
      .getFullUserDetails(
          IdMinimalUser(contact.identityId, None), identityRequest)
      .map(_.privateFields
            .flatMap(_.country)
            .flatMap(CountryGroup.countryByNameOrCode))

    val upgradeCommand = (country |@| newRatePlan |@| oldRest) {
      case (ctry, newPln, restSub) =>
        val plansToRemove =
          getRatePlanIdsToRemove(restSub.ratePlans, catalog.find, discountIds)
        val upgrade =
          Amend(sub.id.get, plansToRemove, NonEmptyList(newPln), sub.promoCode)
        promoService.applyPromotion(upgrade, form.promoCode, ctry)
    }

    addressDetails.foreach(
        identityService.updateUserFieldsBasedOnUpgrade(
            contact.identityId, _, identityRequest)
    )
    ...
    (for {
      ...
      _ <- salesforceService
        .updateMemberStatus(
            IdMinimalUser(contact.identityId, None), tier, customerOpt)
        .liftM
  ...
  private def createPaymentMethod(
      contactId: ContactId, customer: Stripe.Customer): Future[UpdateResult] =
    for {
      sub <- subscriptionService.unsafeGet(contactId)
      result <- zuoraService.createCreditCardPaymentMethod(
          sub.accountId, customer)
    } yield result

  private def subOrPendingAmendError[P <: Subscription](
      sub: P): Future[MemberError \/ P] =
    for {
      restSub <- zuoraService
        .getRestSubscription(sub.name)
        .map(
            _.getOrElse(
                throw new Exception(s"Sub ${sub.name} not found with rest"))
        )
    } yield {
