...
case class IdentityService(identityApi: IdentityApi) {
  def getIdentityUserView(
      user: IdMinimalUser,
      identityRequest: IdentityRequest): Future[IdentityUser] =
    getFullUserDetails(user, identityRequest)
      .zip(doesUserPasswordExist(identityRequest))
      .map {
        case (fullUser, doesPasswordExist) =>
          IdentityUser(fullUser, doesPasswordExist)
      }

  def getFullUserDetails(
      user: IdMinimalUser, identityRequest: IdentityRequest): Future[IdUser] =
    identityApi
      .get(s"user/${user.id}",
           identityRequest.headers,
           identityRequest.trackingParameters)
      .map(_.getOrElse(throw IdentityServiceError(
                  s"Couldn't find user with ID ${user.id}")))

  def doesUserPasswordExist(
      identityRequest: IdentityRequest): Future[Boolean] =
    identityApi.getUserPasswordExists(
        identityRequest.headers, identityRequest.trackingParameters)

  def updateUserFieldsBasedOnJoining(user: IdMinimalUser,
                                     formData: JoinForm,
                                     identityRequest: IdentityRequest) {

    val billingDetails = formData match {
      case billingForm: PaidMemberJoinForm =>
        val billingAddressForm =
          billingForm.billingAddress.getOrElse(billingForm.deliveryAddress)
  ...
  def updateUserPassword(
      password: String, identityRequest: IdentityRequest, userId: String) {
    val json = Json.obj("newPassword" -> password)
    identityApi.post("/user/password",
                     Some(json),
                     identityRequest.headers,
                     identityRequest.trackingParameters,
  ...
  def updateUserFieldsBasedOnUpgrade(userId: String,
                                     addressDetails: AddressDetails,
                                     identityRequest: IdentityRequest) {
    val billingAddressForm =
      addressDetails.billingAddress.getOrElse(addressDetails.deliveryAddress)
    val fields =
      deliveryAddress(addressDetails.deliveryAddress) ++ billingAddress(
          billingAddressForm)
    val json = Json.obj("privateFields" -> fields)
  ...
  def updateEmail(
      user: IdMinimalUser, email: String, identityRequest: IdentityRequest) = {
    val json = Json.obj("primaryEmailAddress" -> email)
    postFields(json, user.id, identityRequest)
  }

  def reauthUser(
      email: String, password: String, identityRequest: IdentityRequest) = {
    val params =
      ("email" -> email) :: ("password" -> password) :: identityRequest.trackingParameters
    identityApi.post("auth", None, identityRequest.headers, params, "reauth")
  }

  private def postFields(
      json: JsObject, userId: String, identityRequest: IdentityRequest) = {
...
trait IdentityApi {
  ...
  def getUserPasswordExists(
      headers: List[(String, String)],
      parameters: List[(String, String)]): Future[Boolean] = {
    ...
    Timing.record(IdentityApiMetrics, "get-user-password-exists") {
      WS
        ...
        .get()
        .map { response =>
          recordAndLogResponse(
              response.status, "GET user-password-exists", endpoint)
          (response.json \ "passwordExists")
            .asOpt[Boolean]
            .getOrElse(
                throw new IdentityApiError(s"$url did not return a boolean"))
        }
    }
  }
  ...
  def get(endpoint: String,
          headers: List[(String, String)],
          parameters: List[(String, String)]): Future[Option[IdUser]] = {
    Timing.record(IdentityApiMetrics, "get-user") {
      ...
      WS
        ...
        .map { response =>
          ...
          if (jsResult.isError)
            Logger.error(
                s"Id Api response on $url : ${response.json.toString}")
          jsResult.asOpt
        }
        .recover {
          case e =>
  ...
  def post(endpoint: String,
           ...
           metricName: String): Future[Int] = {
    Timing.record(IdentityApiMetrics, metricName) {
      val requestHolder = WS
        .url(s"${Config.idApiUrl}/$endpoint")
        .withHeaders(headers:_*)
        .withQueryString(parameters:_*)
        .withRequestTimeout(5000)
      val response = requestHolder.post(data.getOrElse(JsNull))
      response.foreach(
          r => recordAndLogResponse(r.status, s"POST $metricName", endpoint))
      response.map(_.status)
    }
  }

  private def recordAndLogResponse(
      status: Int, responseMethod: String, endpoint: String) {
