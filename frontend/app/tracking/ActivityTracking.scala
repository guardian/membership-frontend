...
case class MemberActivity(source: String, member: MemberData)
    extends TrackerData {
  def toMap: JMap[String, Object] =
    Map("eventSource" -> source) ++ member.toMap
}

case class EventActivity(source: String,
                         member: Option[MemberData],
                         eventData: EventData,
                         order: Option[OrderData] = None) extends TrackerData {
  def toMap: JMap[String, Object] =
    Map("eventSource" -> source) ++ eventData.toMap ++ member.fold(
        ActivityTracking.setSubMap(Map.empty))(_.toMap) ++ order.fold(
        ActivityTracking.setSubMap(Map.empty))(_.toMap)
}
...
case class UpgradeAmendment(tierFrom: Tier,
                            tierTo: PaidTier,
                            effectiveFromDate: Option[DateTime] = Some(
                                  DateTime.now)) extends TierAmendment

case class DowngradeAmendment(
    tierFrom: Tier, effectiveFromDate: Option[DateTime] = None)
    extends TierAmendment {
...
case class MemberData(salesforceContactId: String,
  ...
  def toMap: JMap[String, Object] = {
    ...
    def bcrypt(string: String) =
      (string + Config.bcryptPepper).bcrypt(Config.bcryptSalt)
    ...
    val dataMap =
      Map(
          ...
          "tier" -> tier.name
      ) ++ deliveryPostcode.map("deliveryPostcode" -> truncatePostcode(_)) ++ billingPostcode
        .map("billingPostcode" -> truncatePostcode(_)) ++ subscriptionPlan.map(
          "subscriptionPlan" -> _) ++ city.map("city" -> _) ++ country
        .map("country" -> _) ++ marketingChoices.map { mc =>
...
case class EventData(event: RichEvent) {
  ...
  def toMap: JMap[String, Object] = {
    ...
    val dataMap =
      Map(
          ...
          "group" -> group
      ) ++ Map("tags" -> seqAsJavaList(event.tags)) ++ event.internalTicketing
        .map("isFree" -> _.isFree) ++ event.internalTicketing
        .map("ticketsSold" -> _.ticketsSold) ++ event.internalTicketing.map(
          "saleEnds" -> _.salesEnd.getMillis) ++ event.internalTicketing.map(
          "isCurrentlyAvailableToPaidMembersOnly" -> _.isCurrentlyAvailableToPaidMembersOnly) ++ event.internalTicketing
        .flatMap(_.generalReleaseTicketOpt)
        .map("generalReleaseTicketOpt" -> ticketClassToMap(_)) ++ event.internalTicketing
        .flatMap(_.memberBenefitTicketOpt)
        .map("memberBenefitTicketOpt" -> ticketClassToMap(_)) ++ event.venue.address
        .flatMap(a => a.postal_code)
        .map("postCode" -> _) ++ event.providerOpt.map("provider" -> _)
  ...
  private def ticketClassToMap(
      ticketClass: EBTicketClass): JMap[String, Object] = {
    val dataMap =
      Map(
          "id" -> ticketClass.id,
          ...
          "durationBeforeSaleEnds" ->
          (ticketClass.sales_end.getMillis - DateTime.now.getMillis)
      ) ++ ticketClass.sales_start.map("salesStart" -> _.getMillis) ++ ticketClass.sales_start
        .map(s =>
            "durationAfterSaleStart" -> (DateTime.now.getMillis - s.getMillis)) ++ ticketClass.cost
        .map("value" -> _.value) ++ ticketClass.cost.map("formattedPrice" -> _.formattedPrice
            .replace("£", "")) ++ ticketClass.hidden.map("hidden" -> _)
...
trait ActivityTracking {
  ...
  def trackAnon(data: TrackerData)(implicit request: RequestHeader) {
    val analyticsOff =
      request.cookies.get(Testing.AnalyticsCookieName).isDefined
  ...
  def trackRegistration(formData: JoinForm,
                        tier: Tier,
                        member: ContactId,
                        user: IdMinimalUser,
    ...
    val subscriptionPaymentAnnual = formData match {
      case paidMemberJoinForm: PaidMemberJoinForm =>
        Some(paidMemberJoinForm.payment.billingPeriod.annual)
      case _ => None
    }

    val billingPostcode = formData match {
      case paidMemberJoinForm: PaidMemberJoinForm =>
        paidMemberJoinForm.billingAddress
          .map(_.postCode)
          .orElse(Some(formData.deliveryAddress.postCode))
      case _ => None
    }

    val trackingInfo = MemberData(
  ...
  def trackUpgrade(
      member: GenericSFContact,
      subscription: Subscription with PaymentStatus[MembershipPlan[
              Status, Tier]],
    ...
    track(MemberActivity(
              source = "membershipUpgrade",
              MemberData(
                  salesforceContactId = member.salesforceContactId,
                  identityId = member.identityId,
                  tier = subscription.plan.tier,
                  tierAmendment = Some(UpgradeAmendment(subscription.plan.tier,
                                                        newRatePlan.tier)),
                  deliveryPostcode = addressDetails
                      .map(_.deliveryAddress.postCode),
                  billingPostcode = addressDetails
                      .flatMap(f => f.billingAddress.map(_.postCode))
                      .orElse(addressDetails.map(_.deliveryAddress.postCode)),
                  subscriptionPaymentAnnual = Some(
                        newRatePlan.billingPeriod.annual),
