...
trait Event
    extends Controller with MemberServiceProvider with ActivityTracking {

  val guLiveEvents: EventbriteService
  val localEvents: EventbriteService
  val masterclassEvents: EventbriteService

  private def recordBuyIntention(eventId: String) =
    new ActionBuilder[Request] {
      override def invokeBlock[A](
        ...
        EventbriteService
          .getEvent(eventId)
          .map { event =>
            trackAnon(EventActivity(
                    "buyActionInvoked", None, EventData(event)))(request)
  ...
  private def BuyAction(id: String) =
    NoCacheAction andThen recordBuyIntention(id) andThen authenticated(
        onUnauthenticated = notYetAMemberOn(_)) andThen subscriptionRefiner()

  def details(slug: String) = CachedAction { implicit request =>
    val eventOpt = for {
      id <- EBEvent.slugToId(slug)
      correctEvent <- (Eventbrite.HiddenEvents.get(id).toSeq :+ id)
        .flatMap(EventbriteService.getEvent)
        .headOption
    } yield {
      if (slug == correctEvent.slug) {
        trackAnon(
            EventActivity("viewEventDetails", None, EventData(correctEvent)))
   ...
   */
  def embedData(slug: String) =
    Cors.andThen(CachedAction) { implicit request =>
  ...
  def embedCard(slug: String) = CorsPublicCachedAction { implicit request =>
    ...
    Ok(eventOpt.fold {
      ...
      Json.obj("status" -> "success",
               "html" -> views.html.embeds.eventCard(event).toString())
    })
  }
    ...
    EventbriteService
      ...
      .map {
        case event@(_: GuLiveEvent | _: LocalEvent) =>
          if (event.isBookableByTier(
                  request.subscriber.subscription.plan.tier))
  ...
  private def redirectToEventbrite(event: RichEvent)(
      implicit request: SubscriptionRequest[AnyContent] with Subscriber
      ): Future[Result] =
    Timing.record(
        event.service.wsMetrics,
        s"user-sent-to-eventbrite-${request.subscriber.subscription.plan.tier}") {

      memberService.createEBCode(request.subscriber, event).map { code =>
        val eventUrl = code
          .fold(Uri.parse(event.url))(c => event.url ? ("discount" -> c.code))
        ...
        track(EventActivity(
                  "redirectToEventbrite", Some(memberData), EventData(event)),
              request.user)

        Found(eventUrl).withCookies(Cookie(eventCookie(event), "", Some(3600)))
      }
    }

  private def trackConversionToThankyou(event: RichEvent,
                                        order: Option[EBOrder],
                                        member: Option[Member])(
      implicit request: Request[_]) {

    val memberData = member.map(m =>
          MemberData(salesforceContactId = m.contact.salesforceContactId,
                     identityId = m.contact.identityId,
                     tier = m.subscription.plan.tier,
                     campaignCode = CampaignCode.fromRequest))

    trackAnon(EventActivity("eventThankYou",
                            memberData,
                            EventData(event),
                            order.map(OrderData)))(request)
  }

  def thankyou(id: String, orderIdOpt: Option[String]) =
      ...
      orderIdOpt.fold {
        val resultOpt = for {
          ...
          event.service.getOrder(oid).map { order =>
            ...
            if (count > 0) {
              memberService.recordFreeEventUsage(
                  request.subscriber.subscription, event, order, count)
            }

            trackConversionToThankyou(
                event, Some(order), Some(request.subscriber))
        ...
        Future.successful(Redirect(routes.Event.thankyou(id, None))
              .flashing("oid" -> orderId))
      }
    }
  ...
  def preview(id: String) =
    GoogleAuthenticatedStaffAction.async { implicit request =>
      EventbriteService.getPreviewEvent(id).map(eventDetail)
    }

  def previewLocal(id: String) =
    GoogleAuthenticatedStaffAction.async { implicit request =>
      EventbriteService.getPreviewLocalEvent(id).map(eventDetail)
    }

  def previewMasterclass(id: String) =
    GoogleAuthenticatedStaffAction.async { implicit request =>
