...
object TierController
    extends Controller with ActivityTracking with CatalogProvider
    with SubscriptionServiceProvider with MemberServiceProvider
    with StripeServiceProvider with PaymentServiceProvider {

  def downgradeToFriend() = PaidSubscriptionAction { implicit request =>
    Ok(views.html.tier.downgrade
          .confirm(request.subscriber.subscription.plan.tier, catalog))
  }

  def downgradeToFriendConfirm = PaidSubscriptionAction.async {
    implicit request =>
      // POST
      for {
        cancelledSubscription <- memberService.downgradeSubscription(
            request.subscriber)
  ...
  def downgradeToFriendSummary = PaidSubscriptionAction { implicit request =>
    val startDate = request.subscriber.subscription.chargedThroughDate
      .map(_.plusDays(1))
    ...
    Ok(views.html.tier.downgrade.summary(
            ...
            catalog.friend,
            startDate)).discardingCookies(TierChangeCookies.deletionCookies:_*)
  }
  ...
  def change() = SubscriptionAction { implicit request =>
    ...
    implicit val c = catalog
    Ok(views.html.tier
          .change(request.subscriber.subscription.plan.tier, catalog))
  }

  def handleErrors(memberResult: Future[MemberError \/ _])(
      success: => Result): Future[Result] = memberResult.map { res =>
    handleResultErrors(res.map(_ => success))
  }
  ...
  def cancelPaidTierSummary = PaidSubscriptionAction { implicit request =>
    implicit val c = catalog
    Ok(views.html.tier.cancel.summaryPaid(
            request.subscriber.subscription,
            request.subscriber.subscription.paidPlan.tier))
      .discardingCookies(TierChangeCookies.deletionCookies:_*)
  }

  def upgrade(target: PaidTier) =
    ChangeToPaidAction(target).async { implicit request =>
      implicit val c = catalog
      ...
      val identityUserFieldsF = IdentityService(IdentityApi)
      ...
      def pageInfo(pf: PrivateFields, billingPeriod: BillingPeriod): PageInfo = {
        val selectedCountry = pf.billingCountry.orElse(pf.country).flatMap {
          name =>
      ...
      def fromPaid(subscriber: PaidMember, card: PaymentCard): Future[Result] = {
        ...
        val targetPlanId = targetPlans
          .get(subscriber.subscription.plan.billingPeriod)
          .productRatePlanId
        val preview = memberService.previewUpgradeSubscription(
            subscriber.subscription, targetPlanId)

        (for {
          billingSchedule <- EitherT(memberService.previewUpgradeSubscription(
                  subscriber.subscription, targetPlanId))
          privateFields <- EitherT(identityUserFieldsF.map(\/.right))
        } yield {
          val summary = PaidToPaidUpgradeSummary(
              billingSchedule, subscriber.subscription, targetPlanId, card)
      ...
      val paymentCard = paymentService.getPaymentCard(
          request.subscriber.subscription.accountId)

      paymentCard flatMap { p =>
        (request.subscriber, p) match {
          case (Subscriber.FreeMember(mem), _) => fromFree(mem)
          case (Subscriber.PaidMember(mem), Some(a: PaymentCard)) =>
            fromPaid(mem, a)
          case _ =>
            throw new IllegalStateException(
                request.subscriber.subscription.accountId +
                " is missing a payment status or card")
        }
      }
    }

  def upgradeConfirm(target: PaidTier) =
    ChangeToPaidAction(target).async { implicit request =>
      val identityRequest = IdentityRequest(request)

      def handleFree(freeMember: FreeMember)(form: FreeMemberChangeForm) = {
        val upgrade =
          memberService.upgradeFreeSubscription(freeMember,
                                                target,
                                                form,
                                                identityRequest,
                                                CampaignCode.fromRequest)
        handleErrors(upgrade) {
          Ok(Json.obj("redirect" -> routes.TierController
                    .upgradeThankyou(target)
                    .url))
        }
      }
      ...
      def handlePaid(paidMember: PaidMember)(form: PaidMemberChangeForm) = {
        val reauthFailedMessage: Future[Result] = Future {
          Redirect(routes.TierController.upgrade(target)).flashing(
              "error" -> s"That password does not match our records. Please try again.")
        }

        def doUpgrade(): Future[Result] = {
          val upgrade =
            memberService.upgradePaidSubscription(paidMember,
                                                  target,
                                                  form,
                                                  identityRequest,
                                                  CampaignCode.fromRequest)
        ...
        for {
          status <- IdentityService(IdentityApi).reauthUser(
              paidMember.contact.email, form.password, identityRequest)
      ...
      val futureResult = request.subscriber match {
        case Subscriber.FreeMember(mem) =>
          freeMemberChangeForm.bindFromRequest
            .fold(redirectToUnsupportedBrowserInfo, handleFree(mem))
        case Subscriber.PaidMember(mem) =>
          paidMemberChangeForm.bindFromRequest
            .fold(redirectToUnsupportedBrowserInfo, handlePaid(mem))
        case a =>
          throw new IllegalStateException(
              s"Inconsistent Salesforce state, contact with id ${a.contact.salesforceContactId}")
      }

      futureResult
        .map(_.discardingCookies(TierChangeCookies.deletionCookies:_*))
        .recover {
