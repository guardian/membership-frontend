...
object Giraffe extends Controller {
  ...
  val social: Set[Social] = Set(
      Twitter(
          "The Panama Papers: how the world's rich and famous hide their money offshore http://www.theguardian.com/news/series/panama-papers?CMP=twt_contribute #panamapapers"),
      Facebook(
          "http://www.theguardian.com/news/series/panama-papers?CMP=fb_contribute")
  )
  ...
  def support = CachedAction { implicit request =>
    val pageInfo = PageInfo(
        ...
        url = request.path,
        image = Some(
              "https://media.guim.co.uk/727ed45d0601dc4fe85df56f6b24140c68145c16/0_0_2200_1320/1000.jpg"),
        ...
        customSignInUrl = Some(
              (Config.idWebAppUrl / "signin") ? ("skipConfirmation" -> "true"))
    )
    ...
    request.session
      ...
      .fold(
          Redirect(routes.Giraffe.support().url, SEE_OTHER)
            ...
            Ok(views.html.giraffe.thankyou(
                    PageInfo(
  ...
  def pay = NoCacheAction.async { implicit request =>
    supportForm
      .bindFromRequest()
      .fold[Future[Result]]({ withErrors =>
        Future.successful(
            BadRequest(JsArray(withErrors.errors.map(k => JsString(k.key)))))
      }, { f =>
        val metadata =
          Map(
            ...
            .map("idUser" -> _.user.id) ++ f.postCode.map("postcode" -> _)
        val res = stripe.Charge.create(Math.min(5000, (f.amount * 100).toInt),
                                       f.currency,
                                       f.email,
                                       "Your contribution",
                                       f.token,
