 ...
 */
class CacheSensitiveCSRFFilter(tokenName: => String = CSRFConf.TokenName,
  ...
  def apply(next: EssentialAction): EssentialAction = new CacheSensitiveCSRFAction(next, tokenName, cookieName, secureCookie,
    createIfNotFound, tokenProvider, errorHandler)
}

object CacheSensitiveCSRFFilter {
  def apply(tokenName: => String = CSRFConf.TokenName,
    ...
    new CacheSensitiveCSRFFilter(tokenName, cookieName, secureCookie, createIfNotFound, tokenProvider, errorHandler)
  }
