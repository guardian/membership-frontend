...
object Catalog {
  case class Diagnostic(
      comparisonTable: Option[ComparisonTable], errorTables: Seq[ErrorTable])

  object Diagnostic {
    import MembershipCatalog.Val
    def fromCatalogs(testCat: Val[MembershipCatalog],
                     normalCat: Val[MembershipCatalog]): Diagnostic = {
      val test = "test"
      val normal = "normal"

      val comparisonTable = (
          ComparisonTable.fromCatalog(testCat, test),
          ComparisonTable.fromCatalog(normalCat, normal)) match {
      ...
      val errorsTables = Seq(ErrorTable.fromCatalog(testCat, test),
                             ErrorTable.fromCatalog(normalCat, normal)).flatten

      Diagnostic(comparisonTable, errorsTables)
    }
  }

  case class PlanDescription(name: String,
                             env: String,
                             productRatePlanId: ProductRatePlanId,
                             prices: String)

  object PlanDescription {
    def fromPlanDetails(env: String)(
        productRatePlanId: ProductRatePlanId,
        plan: MembershipPlan[Status, Tier]): PlanDescription = {
      val prices = plan match {
        case PaidMembershipPlan(_, _, _, _, pricing) =>
          pricing.prices.map(_.pretty).mkString("\n")
        case FreeMembershipPlan(_, _, currencies, _) =>
          currencies.mkString(", ")
      }
  ...
  object ComparisonTable {
    def fromCatalog(catalog: Val[MembershipCatalog],
                    env: String): Option[ComparisonTable] =
      catalog.fold(_ => None,
                   c =>
                     ComparisonTable(c.planMap.toSeq.map {
                       case (prpId, plan) =>
  ...
  object ErrorTable {
    def fromCatalog(
        catalog: Val[MembershipCatalog], env: String): Option[ErrorTable] =
      catalog.fold(
          errs =>
            ErrorTable(
                env,
                errs.map { case (tp, msg) => PlanError(planName(tp), msg) }
            ).some,
          _ => None)
  }
  ...
  private def tierWithStatus(tier: Tier, status: Status) =
    s"${tier.name} ${status.name}"

  implicit val pricingWrites = new Writes[Pricing] {
    override def writes(p: Pricing): JsValue =
      Json.obj(
  ...
  private def tiers(currency: Currency, catalog: MembershipCatalog): JsValue =
    JsObject(
        PaidTier.all.map { tier =>
          val details = catalog.findPaid(tier)
          (details.year.pricing.getPrice(currency),
           details.month.pricing.getPrice(currency),
           tier)
        }.collect {
  ...
  implicit val catalogWrites = new Writes[MembershipCatalog] {
    override def writes(c: MembershipCatalog) =
      JsObject(
          Currency.all.map { cur =>
            cur.toString -> tiers(cur, c)
          }
      )
  }
}
