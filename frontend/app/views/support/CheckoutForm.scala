...
object CheckoutForm extends LazyLogging {
  def forIdentityUser(
      idUser: IdentityUser,
      plans: TierPlans,
      requestCountryGroup: Option[CountryGroup]): CheckoutForm = {
    val (country, desiredCurrency) =
      (requestCountryGroup, idUser.country) match {
        case (Some(cg), _) =>
        ...
        case (_, Some(idCountry)) =>
          val currency = CountryGroup.allGroups
            ...
            .getOrElse {
              logger.warn(
                  s"Could not find country $idCountry in any CountryGroup, defaulting to UK")
              CountryGroup.UK
            }
            .currency
