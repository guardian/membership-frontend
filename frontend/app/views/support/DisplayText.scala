...
object DisplayText {
  implicit class TierCopy(tier: Tier) {
    ...
    def detailsLimited(countryGroup: CountryGroup = UK) =
      (tier match {
        ...
        case Staff() =>
          benefits.filterNot(i =>
                Benefits.partner.toSet.contains(i) ||
                i == Benefits.booksOrTickets)
      }).filterNot { benefit =>
        marketedOnlyToUK(benefit) && countryGroup != UK
      }
    ...
    def highlights(countryGroup: CountryGroup = UK) =
      Highlights.forTier(tier).filterNot { highlight =>
        Highlights.marketedOnlyToUK(highlight) && countryGroup != UK
      }

    val chooseTierPatron = List(Highlight(
            "Get all partner benefits, 6 tickets and 4 books plus invitations  to exclusive behind-the-scenes functions"))
  }

  implicit class PaidTierDetailsCopy(
      paidPlans: PaidMembershipPlans[Current, PaidTier]) {
    ...
    val yearlySavingNote: Option[String] = paidPlans.tier match {
      ...
      case Partner() | Patron() =>
        Some(
            s"1 year membership, ${pricing.yearlySavingsInMonths} months free")
      case _ => None
    }
  }
