...
case class Pricing(yearly: Price, monthly: Price) {
  require(yearly.currency == monthly.currency,
          "The yearly and monthly prices should have the same currency")
  ...
  lazy val yearlySavingsInMonths =
    (yearly.amount - yearlyMonthlyPrice.amount) / monthly.amount
...
object Pricing {
  implicit class WithPricing(plans: PaidMembershipPlans[Current, PaidTier]) {
    ...
    def unsafePriceByCurrency(currency: Currency) =
      pricing(currency).getOrElse {
        val ratePlanIds = Seq(plans.month.productRatePlanId,
                              plans.year.productRatePlanId).mkString(",")
    ...
    def pricingByCurrencyOrGBP(currency: Currency) =
      pricing(currency).getOrElse(gbpPricing)
