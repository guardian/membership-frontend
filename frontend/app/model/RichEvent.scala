...
object RichEvent {
  ...
  def groupEventsByDay(
      events: Seq[RichEvent])(implicit ordering: Ordering[LocalDate]
      ): SortedMap[LocalDate, Seq[RichEvent]] = {
    SortedMap(events.groupBy(_.start.toLocalDate).toSeq:_*)(ordering)
  }

  def groupEventsByDayAndMonth(events: Seq[RichEvent])(
      implicit ordering: Ordering[LocalDate]
      ): SortedMap[LocalDate, SortedMap[LocalDate, Seq[RichEvent]]] = {
    SortedMap(groupEventsByDay(events)(ordering)
          .groupBy(_._1.withDayOfMonth(1))
          .toSeq:_*)(ordering)
  }

  def groupEventsByMonth(
      events: Seq[RichEvent]): SortedMap[LocalDate, Seq[RichEvent]] = {
  ...
  def getCitiesWithCount(events: Seq[RichEvent]): Seq[(String, Int)] = {
    ...
    cities.groupBy(identity).mapValues(_.size).toSeq.sortBy {
      case (name, size) => name
    }
  }

  case class GridImage(assets: List[Grid.Asset],
                       metadata: Grid.Metadata,
                       master: Option[Asset]) {
    lazy val masterOrBestAsset =
      master orElse assets.sortBy(_.pixels).reverse.headOption
  }
  ...
  trait RichEvent {
    ...
    def countComplimentaryTicketsInOrder(order: EBOrder): Int = {
      val ticketIds = event.internalTicketing
        .map(_.complimentaryTickets)
        .getOrElse(Nil)
        .map(_.id)
      order.attendees
        .count(attendee => ticketIds.contains(attendee.ticket_class_id))
    }
    ...
    def isBookableByTier(tier: Tier): Boolean =
  ...
  abstract class LiveEvent(
    ...
    val fallbackHighlightsMetadata = HighlightsMetadata(
        "View highlights of past events",
        Links.membershipFront + "#recent-events")
    val highlight = contentOpt
      .map(c => HighlightsMetadata("Read more about this event", c.webUrl))
  ...
  case class MasterclassEvent(
    ...
    val socialImgUrl = imgOpt.map(_.defaultImage)
    val schema = EventSchema.from(this)
    val tags = event.description
      .map(_.html)
      .flatMap(MasterclassEvent.extractTags)
      .getOrElse(Nil)
  ...
  object MasterclassEvent {
    case class tagItem(
        categoryName: String, subCategories: Seq[String] = Seq())
    ...
    def encodeTag(tag: String) =
      tag.toLowerCase.replace("-", "_").replace(" ", "-")
    def decodeTag(tag: String) =
      tag.capitalize.replace("-", " ").replace("_", "-")

    def extractTags(s: String): Option[Seq[String]] =
      "<!--\\s*tags:(.*?)-->".r
        .findFirstMatchIn(s)
        .map(_.group(1).split(",").toSeq.map(_.trim.toLowerCase))
  }
