...
object Eventbrite {
  ...
  case class GuLiveEvent(event: EBEvent, image: Option[EventImage]) extends RichEvent {
    ...
    val socialImgUrl = {
      ...
      image.fold(Config.gridConfig.fallbackImageUrl) { img =>
        val asset = img.assets.find(_.dimensions.width == availableWidths.max)
        asset.map(_.file).getOrElse(Config.gridConfig.fallbackImageUrl)
      }
    }
