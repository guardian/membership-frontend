...
object Benefits {
  ...
  val offersCompetitions = Benefit(
      "offers_competitions", "Offers and competitions")
  ...
  val booksOrTickets = Benefit(
      "books_or_tickets",
      s"$zuoraFreeEventTicketsAllowance tickets or 4 books",
      isNew = true)
  val booksAndTickets = Benefit(
      "books_and_tickets",
      s"$zuoraFreeEventTicketsAllowance tickets and 4 books",
      isNew = true)

  val discount = Benefit("discount", "20% discount for you and a guest")
  val uniqueExperiences = Benefit(
      "unique_experiences", "Exclusive behind-the-scenes functions")

  val marketedOnlyToUK =
    Set[Benefit](liveStream, accessTicket, offersCompetitions)
