...
package object actions {
  ...
  implicit class RichAuthRequest[A](req: AuthRequest[A])
    ...
    def forMemberOpt[T](f: Option[Contact] => T)(
        implicit executor: ExecutionContext): Future[T] =
      Timing
        .record(MemberAuthenticationMetrics, s"${req.method} ${req.path}") {
        for {
          memberOpt <- touchpointBackend.salesforceService
            .getMember(req.user.id)
        } yield f(memberOpt)
      }
  }
  ...
  case class SubscriptionRequest[A](touchpointBackend: TouchpointBackend,
      ...
      extends AuthRequest(request.user, request) with BackendProvider {

    def this(other: SubscriptionRequest[A]) = this(
        other.touchpointBackend, other.request)
  }

  trait Subscriber {
    def paidOrFreeSubscriber: FreeMember \/ PaidMember
    def subscriber: Member =
      paidOrFreeSubscriber.fold[Member](identity, identity)
  }
  ...
  case class IdentityGoogleAuthRequest[A](
      googleUser: googleauth.UserIdentity, request: AuthRequest[A])
