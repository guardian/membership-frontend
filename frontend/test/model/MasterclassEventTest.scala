...
class MasterclassEventTest extends Specification {
  ...
  "MasterclassEvent" should {
    "extract tags when they are present" in {
      val event = Resource.getJson("model/eventbrite/event-with-tag.json").as[EBEvent].toAssumedEventWithDescription
    ...
    "return an empty list when tags are not present" in {
      val event = Resource.getJson("model/eventbrite/event-without-tag.json").as[EBEvent].toAssumedEventWithDescription
      val mcEvent = MasterclassEvent(event, None)
