...
object EventbriteTestObjects {
  ...
  def eventAddress = new EBAddress(None, None, None, None, None)
  def eventVenue = new EBVenue(None, eventAddress, None, None, None)
  def eventWithName(name:String = "") = EBEvent(eventName(name), eventDescription(), "", "", eventTime, eventTime, eventVenue)
}
