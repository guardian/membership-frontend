...
trait EventbriteService {
  ...
  val forEventList: String
  val eventUrl: String
  val token: (String, String)

  def getAllEvents: Future[Seq[EBEvent]] = eventbriteRequest(forEventList).map(asEBEventSequence(_))

  def getEvent(id: String): Future[EBEvent] = eventbriteRequest(forEventWith(id)).map(asEBEvent(_))

  def eventbriteRequest(url: String): Future[Response] = WS.url(url).withQueryString(token).get()

  private def forEventWith(id: String) = eventUrl + s"/$id"
...
object EventbriteService extends EventbriteService {
  ...
  override val forEventList: String = config.getString("eventbrite.event-list-url")
